          ...
          <tr>
             <%
		           ...
		           Statement st = con.createStatement();
				    ResultSet rs;
				    rs = st.executeQuery("select * from hocvientinnhan ");
				    while(rs.next()){
						%>
						<tr>
						<td><%=rs.getString("hocvien") %></td>
						<td><%=rs.getString("mahocvien") %></td>
						<td><%=rs.getString("ngaysinh") %></td>
						<td><%=rs.getString("noidung") %></td>
						<td><a  href="admintraloi.jsp?mahocvien=<%=rs.getString("mahocvien") %>" role="button">Trả lời</a></td>
