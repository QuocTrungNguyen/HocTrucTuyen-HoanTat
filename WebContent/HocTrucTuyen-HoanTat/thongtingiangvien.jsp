  ...
  <div class="col-sm-9 text-left">
        ...
        <%
		           ...
		           Statement st = con.createStatement();
				    ResultSet rs;
				    rs = st.executeQuery("select  from huydangkikh ");
				    while(rs.next()){
						%>
						<tr>
						<td><%=rs.getString("makh") %></td>
						<td><%=rs.getString("tenkhoahoc") %></td>
						<td><%=rs.getString("ngaybatdau") %></td>
						<td><%=rs.getString("thongtin") %></td>
						<td><%=rs.getString("phong") %></td>
						<td><%=rs.getString("ngayketthuc") %></td>
						<td><a href="hocvienxoadanhsachdangki.jsp?makh=<%=rs.getString("makh") %>" >Hủy đăng kí </a></td>
						</tr>
						<% 
						}
				%>
